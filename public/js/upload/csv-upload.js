...
class UploadPage {
    ...
    async handleCsvUpload(event) {
        ...
        if (!file.name.toLowerCase().endsWith('.csv')) {
            Utils.showMessage('upload-message', 'Please select a valid CSV file', 'error');
            return;
        }

        // Validate file size (5MB limit)
        if (file.size > 5 * 1024 * 1024) {
            Utils.showMessage('upload-message', 'File size must be less than 5MB', 'error');
            return;
        }
        ...
        try {
            ...
            Utils.showMessage('upload-message', `Successfully processed ${data.filename}`, 'success');
            this.displayUploadResults(data.results);
        } catch (error) {
            Utils.showMessage('upload-message', error.message || 'Upload failed', 'error');
        } finally {
    ...
    displayUploadResults(results) {
        ...
        if (elements.successfulAdded) elements.successfulAdded.textContent = results.successfullyAdded;
        if (elements.duplicatesIgnored) elements.duplicatesIgnored.textContent = results.duplicatesIgnored;
        if (elements.errorsCount) elements.errorsCount.textContent = results.errorsCount;
