...
class TransactionsPage {
    ...
    setupEventListeners() {
        ...
        if (searchInput) {
            searchInput.addEventListener('input', Utils.debounce(() => this.applyFilters(), 500));
        }
    }
    ...
    async loadTransactions(filters = {}) {
        try {
            ...
            this.updateDateRangeIndicator(isDefaultDateRange, filters.startDate, filters.endDate);
    ...
    clearFilters() {
        const filterIds = ['search-filter', 'type-filter', 'amount-min', 'amount-max', 'date-start', 'date-end'];
    ...
    updateDateRangeIndicator(isDefaultRange, startDate, endDate) {
        ...
        if (indicator && isDefaultRange && startDate && endDate) {
            if (startDateElement) startDateElement.textContent = Utils.formatDateForDisplay(startDate);
            if (endDateElement) endDateElement.textContent = Utils.formatDateForDisplay(endDate);
